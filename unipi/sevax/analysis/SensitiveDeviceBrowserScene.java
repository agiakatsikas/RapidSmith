 ...
 */
public class SensitiveDeviceBrowserScene extends SensitiveTileScene{
	/**	 */
	...
	/**	 */
	private SensitiveDeviceBrowser browser;
	/**	 */
	private Tile reachabilityTile;
	/**	 */
	private ArrayList<SensitiveNumberedHighlightedTile> currentTiles = new ArrayList<SensitiveNumberedHighlightedTile>();
	
	
	public SensitiveDeviceBrowserScene(Device device, WireEnumerator we, boolean hideTiles, boolean drawPrimitives, SensitiveDeviceBrowser browser, String workingDirectory, boolean viewEbd, boolean viewSeu){
	...
	public void drawWire(Tile src, int wireSrc, Tile dst, int wireDst){
		...
		SensitiveWireConnectionLine line = new SensitiveWireConnectionLine(x1,y1,x2,y2, this, dst, wireDst);
	...
	private void drawReachability(HashMap<Tile, Integer> map){
		...
		for(Tile t : map.keySet()){
			...
			SensitiveNumberedHighlightedTile tile = new SensitiveNumberedHighlightedTile(t, this, map.get(t));
	...
	private void menuReachabilityClear(){
		for(SensitiveNumberedHighlightedTile rect : currentTiles){
			rect.remove();
		}
		currentTiles.clear();
	}
