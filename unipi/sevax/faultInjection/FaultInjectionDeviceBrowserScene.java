...
import edu.byu.ece.rapidSmith.device.browser.WireConnectionLine;
 ...
 */
public class FaultInjectionDeviceBrowserScene extends FaultInjectionTileScene{
	/**	 */
	...
	/**	 */
	private FaultInjectionDeviceBrowser browser;
	/**	 */
	private Tile reachabilityTile;
	/**	 */
	private ArrayList<FaultInjectionNumberedHighlightedTile> currentTiles = new ArrayList<FaultInjectionNumberedHighlightedTile>();
	
	
	public FaultInjectionDeviceBrowserScene(Device device, WireEnumerator we, boolean hideTiles, boolean drawPrimitives, FaultInjectionDeviceBrowser browser, String workingDirectory, boolean viewEbd, boolean viewSeu){
		super(device, hideTiles, drawPrimitives, workingDirectory, viewEbd, viewSeu);
	...
	public void drawWire(Tile src, int wireSrc, Tile dst, int wireDst){
		...
		FaultInjectionWireConnectionLine line = new FaultInjectionWireConnectionLine(x1,y1,x2,y2, this, dst, wireDst);
	...
	private void drawReachability(HashMap<Tile, Integer> map){
		...
		for(Tile t : map.keySet()){
			...
			FaultInjectionNumberedHighlightedTile tile = new FaultInjectionNumberedHighlightedTile(t, this, map.get(t));
	...
	private void menuReachabilityClear(){
		for(FaultInjectionNumberedHighlightedTile rect : currentTiles){
			rect.remove();
		}
		currentTiles.clear();
	}
